...
Polymarket予測市場 + テクニカル分析を組み合わせた仮想通貨トレードボット。
5つの戦略ボットが協調して KuCoin 上で自動売買を行う。GPTコスト実質0。

## ボット構成

| ボット | 戦略 | 対象ペア | 間隔 | 資金比率 |
|---|---|---|---|---|
| **Momentum** | コア: EMA(20/50) クロス + MACD / 参考(2/4): 出来高 + ATR + 4h MTF + ADX | BTC/USDT, ETH/USDT | 毎時 | 30% |
| **Momentum Fast** | コア: EMA(9/21) クロス + MACD / 参考(1/2): 出来高 + ATR | BTC/USDT, ETH/USDT | 15分毎 | 10% |
| **Range** | コア: RSI(30/70) 反転 + BB外側 / 参考(2/3): BB幅 + ADX + Polymarket確率 | XRP/USDT, SOL/USDT | 15分毎 | 35% |
...
- 全ボット GPT 不要（Polymarket + テクニカル指標で完結）
...
| エントリー（参考） | 出来高 ≥ 1.2倍 / ATR 拡大 ≥ 1.1倍 / 4h EMA(50) 方向一致 / ADX > 25（4つ中2つ以上） |
...
| エントリー（参考） | 出来高 ≥ 1.2倍 / ATR 拡大（2つ中1つ以上） |
...
| エントリー（参考） | BB幅 < 6% / ADX ≤ 25 / Polymarket確率フィルター（3つ中2つ以上） |
...
| HALT | RSSニュースにHALTキーワード（hack, exploit, ban等）検出で全ボット停止 |
...
│   ├── indicators.ts        EMA 計算（共通モジュール）
│   ├── risk.ts              トレーリングストップ・部分利確・ATR・PnL 計算
│   ├── gpt.ts               OpenAI API ラッパー（将来拡張用）
