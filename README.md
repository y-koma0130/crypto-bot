...
| **Sentiment** | GPTセンチメント分析(Polymarket+RSS) + HALTキーワード即時検出（司令塔） | 全4ペア | 30分毎 | 20% |
...
| 項目 | 値 |
|---|---|
| 判定方式 | **GPT analyzeSentimentBatch** でPolymarket確率+RSSニュースを文脈付き解析。キーワードHALTは即時検出（API待ち不要）。GPT失敗時はキーワードベースにフォールバック |
...
| HALT | キーワード検出（hack, exploit等）で即時全ボット停止 + GPTがHALT判定した場合も同様 |
| データソース | Polymarket Gamma API + CoinDesk/CoinTelegraph RSS → GPT-4o-mini で総合判定 |
